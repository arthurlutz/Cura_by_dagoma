...
class CustomRepRapInfoPage(InfoPage):
	def __init__(self, parent):
		...
		# self.AddSeperator()
		self.machineName = self.AddLabelTextCtrl(_("Nom de la Machine"), "RepRap")
		self.machineWidth = self.AddLabelTextCtrl(_("Largeur de la Machine (mm)"), "80")
		self.machineDepth = self.AddLabelTextCtrl(_("Profondeur de la Machine (mm)"), "80")
		self.machineHeight = self.AddLabelTextCtrl(_("Hauteur de la Machine (mm)"), "55")
		self.nozzleSize = self.AddLabelTextCtrl(_("Taille de la buse (mm)"), "0.5")
		self.heatedBed = self.AddCheckbox(_("Plateau chauffant"))
		self.HomeAtCenter = self.AddCheckbox(_("Centre du Plateau en 0,0,0 (RoStock)"))
...
class DiscoveryReadyPage(InfoPage):
	def __init__(self, parent):
		...
		# self.linkweb = hl.HyperLinkCtrl(self, wx.ID_ANY, "www.dagoma.fr", URL="http://www.dagoma.fr/", pos=(103,116))
